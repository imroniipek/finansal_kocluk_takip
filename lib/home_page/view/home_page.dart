            ...
            BlocBuilder<HomePageBloc,HomePageState>(
                ...
                builder:(context,state)
                {
                  return AnimatedContainer(
                    ...
                    height: isOpen ? (context.read<HomePageBloc>().state.incomes.length*60 ) : 0.0,
  ...
  List<Widget> widgetList(BuildContext context) {
    return Sabitler.expensesSelections.entries.toList().asMap().entries.map((indexedEntry)
    {
      ...
      return IconButton(
        ...
        onPressed: () {
          Navigator.push(context, MaterialPageRoute(builder: (context) => IncomeExpansePage(isitIncomepage: false, type: PeriodType.daily, buttonName: entry.value,),),);
        },
      );
    }).toList();
  }
