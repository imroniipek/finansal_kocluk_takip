...
class IncomeExpenseBloc extends Bloc<IncomeExpenseEvent, IncomeExpenseStatus> {
  ...
  IncomeExpenseBloc() : super(IncomeExpenseStatus(title: "Yeni Gelir", date: converttoDate(DateTime.now()) , numbers: [],tempValue: "0"),) {
    ...
    on<AddOperator>((event, emit) {
      ...
      {
        emit(state.copyWith(firstValue: value, operator: event.operator, numbers: [], tempValue:state.tempValue));
      }
      else {
    ...
    on<CalculateResult>((event, emit) {
      ...
      emit(state.copyWith(firstValue: result, tempValue: result.toString(),setFirstOperator: true,operator: null, numbers: [],));
    });
    ...
    on<ClearTheDigit>((event, emit)
    {
      ...
      if(value.length == 1)
      {
        emit(state.copyWith(tempValue: "0", numbers: [], firstValue: 0.0, operator: null,));
        return;
      }
